...
router.get("/get_campaigns", validateUser, checkPlan, async (req, res) => {
  try {
    const campaigns = await query(
      "SELECT * FROM beta_campaign WHERE uid = ? ORDER BY created_at DESC",
      [req.decode.uid]
    );
  ...
  async (req, res) => {
    try {
      ...
      const logs = await query(
        "SELECT * FROM beta_campaign_logs WHERE campaign_id = ? ORDER BY created_at DESC",
        [campaignId]
      );
...
router.get("/dashboard", validateUser, async (req, res) => {
  try {
    ...
    const dailyStats = await query(
      ...
      FROM beta_campaign_logs
      WHERE uid = ? AND created_at > DATE_SUB(NOW(), INTERVAL 30 DAY)
