    ...
    data = await Promise.all(
      data.map(async (x) => {
        const [result] = await query(
          `SELECT COUNT(*) AS count FROM contacts WHERE phonebook_id = ?`,
          [x.id]
        );
...
router.post("/del_phonebook", validateUser, async (req, res) => {
  try {
    ...
    await query(`DELETE FROM contacts WHERE phonebook_id = ? AND uid = ?`, [
      id,
      req.decode.uid,
    ]);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Execute the query
      await query(
        `INSERT INTO contacts (uid, phonebook_id, phonebook_name, name, mobile, var1, var2, var3, var4, var5) VALUES ?`,
        [values]
      );
...
router.get("/get_uid_contacts", validateUser, async (req, res) => {
  try {
    const data = await query(`SELECT * FROM contacts WHERE uid = ?`, [
      req.decode.uid,
    ]);
    res.json({ data, success: true });
  } catch (err) {
...
router.post("/del_contacts", validateUser, async (req, res) => {
  try {
    await query(`DELETE FROM contacts WHERE id IN (?)`, [req.body.selected]);
