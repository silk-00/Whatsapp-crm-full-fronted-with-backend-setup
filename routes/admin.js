...
// get orders (simplified - no orders table exists yet)
router.get("/get_orders", adminValidator, async (req, res) => {
  try {
    // Return empty orders array since orders table doesn't exist yet
    const data = [];
    res.json({ data, success: true });
  } catch (err) {
    console.log(err);
...
// get analytics (alias for dashboard)
router.get("/get_analytics", adminValidator, async (req, res) => {
  try {
    ...
    const getContacts = await query(`SELECT * FROM contacts`, []);
    const getPhonebooks = await query(`SELECT * FROM phonebook`, []);
    const getTemplates = await query(`SELECT * FROM templets`, []);
    const getCampaigns = await query(`SELECT * FROM broadcast`, []);
    const getChatbots = await query(`SELECT * FROM beta_chatbot`, []);
    ...
    res.json({
      data: {
        ...
        contactLength: getContacts.length,
        phonebookLength: getPhonebooks.length,
        templateLength: getTemplates.length,
        campaignLength: getCampaigns.length,
        chatbotLength: getChatbots.length,
        activeInstances,
        orderLength: 0, // No orders table yet
        chatLength: 0, // No chats table yet
      },
      success: true,
    });
  } catch (err) {
...
// get dashboard for user (simplified)
  ...
  try {
    ...
    res.json({
      data: {
        ...
        // Financial data (empty since no orders table)
        orderLength: 0,
        recentTransactions: [],
