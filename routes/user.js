    ...
    if (!compare) {
      ...
      res.json({
        ...
        user: {
          ...
          email: userFind[0].email,
          mobile_with_country_code: userFind[0].mobile_with_country_code,
          role: 'user'
        }
      });
    }
...
router.post("/check_contact", validateUser, async (req, res) => {
  try {
    ...
    const findFirst = await query(
      `SELECT * FROM contacts WHERE mobile = ? AND uid = ? `,
...
router.get("/get_dashboard", validateUser, async (req, res) => {
  try {
    ...
    const user = userData[0] || { uid, name: 'User', email: 'user@example.com' };

    // Get real statistics from database
    const totalContacts = await query(`SELECT COUNT(*) as count FROM contacts WHERE uid = ?`, [uid]);
    const totalCampaigns = await query(`SELECT COUNT(*) as count FROM broadcast WHERE uid = ?`, [uid]);
    const totalTemplates = await query(`SELECT COUNT(*) as count FROM templets WHERE uid = ?`, [uid]);
    const totalChatbots = await query(`SELECT COUNT(*) as count FROM beta_chatbot WHERE uid = ?`, [uid]);
    const totalPhonebooks = await query(`SELECT COUNT(*) as count FROM phonebook WHERE uid = ?`, [uid]);
    const totalFlows = await query(`SELECT COUNT(*) as count FROM beta_flows WHERE uid = ?`, [uid]);

    // Get chat statistics
    const totalChats = await query(`SELECT COUNT(*) as count FROM beta_chats WHERE uid = ?`, [uid]);
    const pendingChats = await query(`SELECT COUNT(*) as count FROM beta_chats WHERE uid = ? AND status = 'pending'`, [uid]);
    const resolvedChats = await query(`SELECT COUNT(*) as count FROM beta_chats WHERE uid = ? AND status = 'resolved'`, [uid]);

    // Get active instances
    const activeInstances = await query(`SELECT COUNT(*) as count FROM instance WHERE uid = ? AND status = 'ACTIVE'`, [uid]);
    ...
    const data = {
      ...
      stats: {
        ...
        totalChatbots: totalChatbots[0]?.count || 0,
        totalPhonebooks: totalPhonebooks[0]?.count || 0,
        totalFlows: totalFlows[0]?.count || 0,
        totalChats: totalChats[0]?.count || 0,
        pending: pendingChats[0]?.count || 0,
        resolved: resolvedChats[0]?.count || 0,
        activeInstances: activeInstances[0]?.count || 0,
        totalBroadcast: totalCampaigns[0]?.count || 0,
        totalTemplets: totalTemplates[0]?.count || 0,
      },
    ...
    res.json({ success: false, msg: "Failed to load dashboard data", error: err.message });
  }
});
